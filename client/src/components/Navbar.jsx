...
const Navbar = () => {
    ...
    const { isLoading, error, data: dataSearch, refetch } = useQuery({
        ...
        queryFn: () => newRequest.get(`/questions?search=${debouncedSearchQuery}`).then((res) => {
            return res.data;
        }),
    ...
    }, [debouncedSearchQuery, refetch, queryClient]);
    ...
    return (
        <div className={` ${location.pathname.includes("/admin") ? "hidden" : "flex"}  z-[100] fixed top-0 left-0 h-16 w-full  bg-white  justify-center  shadow-md`}>
            <div className=" relative px-2 w-full h-full max-w-[1200px] flex gap-4 items-center justify-between">
                ...
                {debouncedSearchQuery && <SearchContent isLoading={isLoading} error={error} data={dataSearch} setSearchQuery={setSearchQuery} />}
                ...
                {options && (
                    <div className='lg:hidden mr-2 absolute bg-white/70 backdrop-blur-md rounded-lg right-0 w-fit top-20 p-4 border-[1px] border-gray-300'>
                        <div className='flex flex-col gap-2 w-full'>
                            <input value={searchQuery} onChange={handleSearch} className='md:hidden text-black w-full border-[1px] border-gray-300 bg-transparent outline-none p-1 rounded-md' type="text" placeholder='Search by Title' />
                            {!user && (
